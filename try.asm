...
_buffer		times 10 db 0						
_bufptr1	dw 0						
_bufptr2 dw 0						

_scantb	db 0,1,'1234567890-=',080h	
		db 0,'qwertyuiop[]',0dh		
		db 0,'asdfghjkl;',27h,'`'	
		db 0,'\zxcvbnm,./',0,'*'	
		db 0,20h,0
...
buffer		equ _buffer  - $$
bufptr1		equ _bufptr1 - $$
bufptr2		equ _bufptr2 - $$
scantb		equ _scantb - $$
...
.021h:  			Gate    SelectorCode32,     NEW_IntHandler,       0, DA_386IGate        ;new interrupt 021h

%rep 94
			Gate	SelectorCode32, SpuriousHandler,      0, DA_386IGate
%endrep

;.7fh:  			Gate    SelectorCode32,     NEW_IntHandler,       0, DA_386IGate        ;new interrupt 021h
...
LABEL_SEG_CODE32:
	...
	;mov	al, [buffer]
	;mov	[gs:((80 * 7 + 70) * 2)], ax	; 屏幕第 7 行, 第 70 列。
...
UserIntHandler	equ	_UserIntHandler - $$
	...
	mov	al, 'A'
	mov	[gs:((80 * 6 + 70) * 2)], ax	; 屏幕第 6 行, 第 70 列。
	iretd
...
;new 07fh interrupt's handler
_NEW_IntHandler:
NEW_IntHandler equ _NEW_IntHandler - $$
   	;mov	ah, 0Ch				; 0000: 黑底    1100: 红字
	;mov	al, 'W'
	;mov	[gs:((80 * 6 + 75) * 2)], ax	; 屏幕第 6 行, 第 75 列。
	;mov	al, 20h
	;out	20h, al				; 发送 EOI
